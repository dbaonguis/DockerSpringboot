...
docker run -d -p 8080:8080 --name spring-boot-jpa-docker-webapp --link demo-mysql:mysql -e DATABASE_HOST=demo-mysql -e DATABASE_PORT=3306 -e DATABASE_NAME=demo -e DATABASE_USER=dbuser -e DATABASE_PASSWORD=dbpass dbaonguis/spring-boot-jpa-docker-webapp